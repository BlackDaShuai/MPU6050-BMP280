...
//静止时刻的补偿值，作用在MPU_Get_Gyroscope()
int errorTuoluoyiX = +111;
int errorTuoluoyiY = -31;
int errorTuoluoyiZ = -10;
//初始化MPU6050
//返回值:0,成功
//    其他,错误代码
u8 MPU_Init(void)
...
u8 MPU_Get_Gyroscope(short *gx, short *gy, short *gz)
{
	...
	if (res == 0)
	{
		*gx = (((u16)buf[0] << 8) | buf[1])+errorTuoluoyiX;
		*gy = (((u16)buf[2] << 8) | buf[3])+errorTuoluoyiY;
		*gz = (((u16)buf[4] << 8) | buf[5])+errorTuoluoyiZ;
	}
	return res;;
}
//得到加速度值(原始值)
